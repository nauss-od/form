...
type DashboardData = {
  ...
  employees: Array<{ id: string; name: string; _count: { courses: number }; submissionCount: number }> | null;
...
export default function DashboardPage() {
  ...
  const loadData = useCallback((role: 'MANAGER' | 'EMPLOYEE', apiRole: string) => {
    const asEmployee = apiRole === 'MANAGER' && role === 'EMPLOYEE';
  ...
  useEffect(() => {
    ...
    setActiveRole(role);
    // Initial load: first get apiRole, then load with correct scope
    fetch('/api/courses?stats=true')
      .then(r => { if (!r.ok) throw new Error(); return r.json(); })
      .then(d => {
        const asEmployee = d.userRole === 'MANAGER' && role === 'EMPLOYEE';
        if (asEmployee) {
          // Re-fetch with employee scope
  ...
  useEffect(() => {
    const handler = () => {
      const role = getActiveRole();
      setActiveRole(role);
      if (data) loadData(role, data.userRole);
    };
  ...
  const showManagerView = isManager && activeRole === 'MANAGER';
  ...
  function refreshCourse() {
    const asEmployee = isManager && activeRole === 'EMPLOYEE';
    fetch(`/api/courses?stats=true${asEmployee ? '&asEmployee=true' : ''}`)
    ...
    return (
      <AppShell title="لوحة المدير" role={appRole}>
        <div className="kpi-grid">
          <div className="kpi-card"><span><IconCourses /> إجمالي الدورات</span><strong>{data.totalCourses}</strong></div>
          <div className="kpi-card"><span><IconUsers /> إجمالي المسجلين</span><strong>{data.totalSubmissions}</strong></div>
          <div className="kpi-card"><span><IconUsers /> عدد الموظفين</span><strong>{emp.length}</strong></div>
          <div className="kpi-card"><span><IconActivity /> موظفون نشطون</span><strong>{emp.filter(e => e._count.courses > 0).length}</strong></div>
        </div>
        <div className="section-card">
          ...
          {!data.recentCourses?.length ? (
            ...
            <div className="course-grid" style={{ gridTemplateColumns: '1fr', gap: 12 }}>
              {data.recentCourses.map(c => <CourseCard key={c.id} c={c} onDeleted={removeCourse} onEdited={refreshCourse} />)}
            </div>
          )}
        </div>
      </AppShell>
  ...
  // Employee view (or manager acting as employee)
  return (
    <AppShell title="لوحة المستخدم" role={appRole}>
      <Link href="/new-course" className="create-hero">
        <div className="create-hero-icon">＋</div>
        <div><strong>إنشاء دورة جديدة</strong><span>احصل على رابط وأرسله للمتدربين</span></div>
